...
class FlowCalibrationRunner:
    ...
    def run(self, gas, flows_lpm, repeats, stroke_start_ml, stroke_end_ml):
        ...
        with (run_dir / 'summary.csv').open('w', newline='', encoding='utf-8') as f:
            w = csv.DictWriter(f, fieldnames=['gas', 'trial_id', 'target_flow_lpm', 'repeat_index', 'actual_flow_lpm', 'mean_voltage_v', 'std_voltage_v', 'sample_count'])
